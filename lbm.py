...
class D2Q9LBM:
    def __init__(self, resolution, tau, u_inlet):
        ...
        cx, cy, r = int(0.15 / resolution), int(0.21 / resolution), int(0.05 / resolution) 
    ...
    def run_animation(self, steps=200, interval=50):
        ...
        fig.suptitle(f"LBM Flow Past a Cylinder (Re ≈ {Re:.1f})", fontsize=16, fontweight='bold')
        ...
        cbar1 = plt.colorbar(vel_plot, ax=ax1, orientation='horizontal', label='Velocity', 
                             shrink=0.8, aspect=40, pad=0.18)
        ...
        cbar2 = plt.colorbar(vort_plot, ax=ax2, orientation='horizontal', label='Vorticity', 
                             shrink=0.8, aspect=40, pad=0.18)
        ...
        def update(frame):
            for _ in range(5):  # Perform multiple steps per frame for smoother animation
                self.step()
